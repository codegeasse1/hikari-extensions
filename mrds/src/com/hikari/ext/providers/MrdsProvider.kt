...
/**
 * MRDS (mrds.com / www.mrds66.com) — daily contest / gossip blacklist video site.
 *
 * Native Hikari extension (no CloudStream dependency). Everything is plain
 * server-rendered HTML:
 *  - home + categories: `https://mrds.com/category/<slug>/` → `.../page/<n>/`
 ...
 *  - post images live on `pic.ndhixj.cn` AES-encrypted (key/iv baked into
 ...
 */
class MrdsProvider : HikariProvider {

    override val id = "mrds"
    override val name = "MRDS"
    override val mainUrl = "https://mrds.com"
    override val description = "Daily contest & gossip videos from mrds.com — 21 category walls plus search, direct HLS streams."
    ...
    companion object {
        private const val BASE = "https://mrds.com"
    ...
    override fun catalogs(): List<HikariCatalog> = listOf(
        ...
        HikariCatalog("ztds", "Theme Contest", HikariMediaType.MOVIE),
        HikariCatalog("rstt", "Hot Search", HikariMediaType.MOVIE),
        HikariCatalog("xazd", "Campus Students", HikariMediaType.MOVIE),
        HikariCatalog("blyp", "Must Watch", HikariMediaType.MOVIE),
        HikariCatalog("fctg", "Leaked Secrets", HikariMediaType.MOVIE),
        HikariCatalog("mhds", "Internet Celebrity", HikariMediaType.MOVIE),
        HikariCatalog("lqdp", "Bizarre", HikariMediaType.MOVIE),
        HikariCatalog("jdsj", "AV Movies", HikariMediaType.MOVIE),
        HikariCatalog("mxwh", "Celebrity Contest", HikariMediaType.MOVIE),
        HikariCatalog("smdh", "Anime & Manga", HikariMediaType.MOVIE),
        HikariCatalog("dypd", "Film & Anime", HikariMediaType.MOVIE),
        HikariCatalog("mtds", "Cosplay", HikariMediaType.MOVIE),
        HikariCatalog("ysds", "ASMR", HikariMediaType.MOVIE),
        HikariCatalog("czds", "Edging Challenge", HikariMediaType.MOVIE),
        HikariCatalog("hjds", "PMV Mix", HikariMediaType.MOVIE),
        HikariCatalog("tgds", "Original Submissions", HikariMediaType.MOVIE),
        HikariCatalog("omjp", "Western Premium", HikariMediaType.MOVIE),
        HikariCatalog("qwcs", "All Network", HikariMediaType.MOVIE),
        HikariCatalog("aijc", "AI Theater", HikariMediaType.MOVIE),
        HikariCatalog("sjbq", "World Cup Zone", HikariMediaType.MOVIE),
    )
    ...
    override suspend fun getMeta(media: HikariMedia): HikariMedia {
        ...
        var poster = Regex("meta itemprop=\"image\" content=\"([^\"]+)\"")
            .find(page)?.groupValues?.get(1)
        if (poster.isNullOrBlank()) {
            poster = Regex("loadBannerDirect\\s*\\(\\s*['\"]([^'\"]+)['\"]")
                .find(page)?.groupValues?.get(1)
        }
        if (poster != null && poster.contains("pic.ndhixj.cn")) {
    ...
    override suspend fun getStreams(media: HikariMedia, episode: HikariEpisode?): List<HikariStream> {
        ...
        // data-config JSON uses escaped slashes: https:\/\/hls.dscxru.cn\/...
        ...
        return urls.mapIndexed { i, u ->
            HikariStream(
                name = if (urls.size == 1) "MRDS" else "Video ${i + 1}",
                url = u,
                headers = headers,
                isM3u8 = true,
            )
    ...
    /** Decrypts the AES-encrypted poster bytes served by pic.ndhixj.cn. */
